...
describe("Unit Conversions from Tables", () => {
  it("generates correct conversion functions", () => {
    ...
    expect(calc("1big + 5medium", config)).toEqual(["1.5big"]);
    expect(calc("1medium + 10small", config)).toEqual(["3medium"]);
    expect(calc("1big + 25small", config)).toEqual(["1.5big"]);
    
    // Test subtraction
    const subConversions = generateConversionsFromTable(table, '-');
    addUnitConversions(config, subConversions);
    
    expect(calc("2big - 5medium", config)).toEqual(["1.5big"]);
    expect(calc("5medium - 10small", config)).toEqual(["3medium"]);
    expect(calc("2big - 25small", config)).toEqual(["1.5big"]);
  });
});
...
describe("Integration with Calculator", () => {
  it("performs calculations with length units", () => {
    ...
    expect(calc("1m + 50cm", config)).toEqual(["1.5m"]);
    expect(calc("10cm + 5mm", config)).toEqual(["10.5cm"]);
    
    // Subtraction
    expect(calc("2km - 500m", config)).toEqual(["1.5km"]);
    expect(calc("1m - 20cm", config)).toEqual(["0.8m"]);
    
    // Complex expressions
    expect(calc("(1km + 200m) / 2", config)).toEqual(["0.6km"]);
    expect(calc("2 * (500m + 50cm)", config)).toEqual(["1001m"]);
  });
  ...
  it("performs calculations with time units", () => {
    ...
    // Addition
    expect(calc("1h + 30min", config)).toEqual(["1.5h"]);
    expect(calc("1min + 30s", config)).toEqual(["1.5min"]);
    
    // Subtraction
    expect(calc("1h - 15min", config)).toEqual(["0.75h"]);
    expect(calc("1min - 15s", config)).toEqual(["0.75min"]);
    
    // Complex expressions
    expect(calc("(2h + 30min) / 2", config)).toEqual(["1.25h"]);
  });
  ...
  it("performs calculations with weight units", () => {
    ...
    // Addition
    expect(calc("1kg + 500g", config)).toEqual(["1.5kg"]);
    expect(calc("1g + 500mg", config)).toEqual(["1.5g"]);
    
    // Subtraction
    expect(calc("2kg - 500g", config)).toEqual(["1.5kg"]);
    
    // Complex expressions
    expect(calc("(1kg + 200g) * 2", config)).toEqual(["2.4kg"]);
  });
});
